...
def get_mentor(mentor_id: int, db: Session = Depends(get_db)):
   ...
   """
   # Fazemos uma consulta, filtramos o ID e pegamos o primeiro resultado. 
   mentor = db.query(models.Mentor).filter(models.Mentor.id == mentor_id).first()
   
   # A logica para o erro 404 continua a mesma aqui 
   if mentor is None:
...
def update_mentor(mentor_id: int, mentor_data: schemas.MentorCreate, db: Session = Depends(get_db)):
   ...
   """
   # Vamos buscar o mentor no banco de dados
   mentor_to_update = db.query(models.Mentor).filter(models.Mentor.id == mentor_id).first()
   
   # Caso o mentor nao exista, iremos retornar 404
   ...
   mentor_to_update.experience_of_years = mentor_data.experience_of_years
   mentor_to_update.linkedin_url = mentor_data.linkedin_url
   mentor_to_update.github_url = mentor_data.github_url
   mentor_to_update.instagram_url = mentor_data.instagram_url
   mentor_to_update.photo_url = mentor_data.photo_url
   mentor_to_update.location = mentor_data.location
